...
- **校正機能の自動実行モード（設定画面トグル）**
  - 現状：投稿編集画面のアイコンをタップした時のみ校正APIが実行される
  - 追加要望：設定画面にトグルを追加し、ONにするとアイコンタップ不要で自動実行
  - 想定仕様：
    - `settingsStore` に `autoProofreading: boolean` を追加
    - 設定画面（翻訳オプション付近 or 新「投稿」セクション）にトグル配置
    - Yahoo JAPAN Client ID 未設定時はグレーアウト
    - 自動モードON時：テキスト入力停止から約2秒後（debounce）に自動で校正API呼び出し
    - アイコンは常時 Primary 色で強調表示（自動モード中であることを示す）
    - 既存のタップ操作は引き続き有効
  - **保留理由**：リリース前に複雑性を持ち込まないため。Yahoo API 呼び出し回数増加の影響も要考慮。ユーザーからの要望次第で追加予定。
