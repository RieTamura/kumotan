...
### 18. ログイン画面のApp Passwordセキュリティノート削除

**目的**: OAuth認証に完全移行済みのため、App Passwordに関するセキュリティノートを削除
...
1. **Security Noteコンポーネントの削除** (`LoginScreen.tsx`):
   - `🔒 App Passwordは認証にのみ使用され、保存されません。`の表示ブロックを削除
   - 関連するスタイル定義（`securityNote`、`securityNoteText`）を削除

2. **翻訳キーの削除** (`login.json`):

```json
// 削除された翻訳キー
"security": {
  "note": "🔒 App Passwordは認証にのみ使用され、保存されません。"  // ja
  "note": "🔒 App Password is used for authentication only and is not stored."  // en
}
```

**効果**:

- OAuth認証一本化に伴い、不要になったApp Password関連の説明文を画面から除去
- ログイン画面がよりシンプルに
...
1. **ステップ1のハイライト対象を単語単位に変更** (`PostCard.tsx`, `HomeScreen.tsx`):
   - 従来はコンテンツ領域全体（`contentColumnRef`）を青枠で囲っていたが、最初の英単語1つのみを囲うように変更
   - ネストされた`Text`の`measureInWindow`が不正確なため、テキストコンテナ`View`の位置から単語幅を推定する方式に変更
   - `firstWordRef`（Text ref）を廃止し、`postTextContainerRef`（View ref）+ `firstWordTextRef`（単語テキスト追跡）に置き換え

2. **ヒントアイコンの青枠サイズ修正** (`HomeScreen.tsx`):
   - `measureTips()`が未呼び出しだった問題を修正し、`useEffect`でチュートリアル開始時に呼び出すように変更
   - Pressableのpadding分をオフセットし、アイコン（24x24）にフィットする青枠を表示
   - フォールバック座標もアイコンサイズに合わせて修正（48x48 → 24x24）

3. **カットアウトオーバーレイの実装** (`TutorialTooltip.tsx`):
   - 全画面を覆う単一の暗いオーバーレイから、ハイライト領域に「穴」を開ける4分割オーバーレイ方式に変更
   - ハイライト対象の上・下・左・右に4つの暗い`View`を配置し、中央は透明（元の明るさ）で表示
   - 追加ライブラリ不要（純粋なView配置）
   - `hasValidHighlight`がfalseの場合は従来通り全画面オーバーレイにフォールバック

4. **「戻る」ボタンの追加** (`useTutorial.ts`, `TutorialTooltip.tsx`, `HomeScreen.tsx`):
   - `prevStep()`関数を追加（ステップ1より前には戻らない）
   - ステップ1では「スキップ」、ステップ2以降では「戻る」を左側に表示
   - 翻訳キー`back`を日本語（「戻る」）・英語（"Back"）で追加

5. **ステップ3の説明文更新** (`tutorial.json`):
   - 「翻訳機能を有効化」の説明に辞書データのダウンロードについて追記
   - 日本語: 「設定画面でDeepL API Keyを登録すると日本語翻訳が、辞書データをダウンロードするとオフライン辞書が使えるようになります。」
   - 英語: "Register a DeepL API Key in Settings to enable Japanese translation, and download dictionary data for offline lookup."

**効果**:
- チュートリアルのハイライト対象が明確になり、ユーザーが操作すべき箇所を直感的に理解できる
- カットアウトにより、暗いオーバーレイ越しでもハイライト対象が元の明るさで視認可能
- 「戻る」ボタンでチュートリアルの前後を確認でき、見逃した説明を再確認可能
