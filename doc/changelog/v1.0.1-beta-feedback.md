...
### 7. ログイン画面の正規OAuth認証対応

**目的**: ユーザーに安全なOAuth認証を優先的に提供しつつ、従来のApp Password認証もオプションとして維持する。

**修正ファイル**: `src/screens/LoginScreen.tsx`

**修正内容**:
- **OAuth優先UI**: 起動時のデフォルト表示をOAuth認証（ハンドル名入力 + ログインボタン）に変更。
- **詳細オプションの隠蔽**: App Password用フォームを「詳細オプション」として折りたたみ可能に実装。
- **共通入力の最適化**: ハンドル名入力フィールドをOAuthとApp Passwordで共用し、送信時の挙動を動的に切り替え。
- **スタイルの追加**: OAuthセクションおよびフォーム切り替え用のリンク（underline表示）を追加。
