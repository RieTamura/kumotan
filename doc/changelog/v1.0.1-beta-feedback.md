...
### 7. ログイン画面の正規OAuth認証対応（入力フォーム一本化）

**目的**: ユーザー様のフィードバックに基づき、OAuth認証時もハンドル名とパスワードを同時に入力する慣習に合わせたUIに改善。

**修正ファイル**: `src/screens/LoginScreen.tsx`

**修正内容**:
- **フォームの一本化**: 「詳細オプション」への切り替えを廃止し、最初からハンドル名とApp Password入力欄を表示する設計に変更。
- **入力バリデーションの強化**: **OAuth認証によるログイン時にもパスワード入力を必須化**。入力不備がある場合はエラーを表示し、ユーザーの意図しない操作を防止。
- **2つのログイン手段の視覚的統合**: App Passwordによる直接ログインと、OAuthによるブラウザログインを「または (or)」の区切り線で明確に分類し、同一画面から選択可能に。
- **ロード状態の最適化**: 入力されたパスワードの形式（ハイフンの有無）に基づき、実行中のフローに応じたローディング表示を行うインテリジェントな挙動を実装。
