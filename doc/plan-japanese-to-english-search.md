...
- [x] 日本語（漢字/ひらがな/カタカナ）で英語訳を取得できる
- [x] 検索結果に品詞情報が含まれる
- [x] JMdictで見つからない場合、DeepLにフォールバック
- [x] 既存の英語→日本語検索に影響なし
- [x] 単体テストカバレッジ 80%以上
...
### Phase 2.5（日本語投稿の単語モード）✅ 完了

- [x] 日本語テキストを単語単位でタップ可能
- [x] 日本語単語タップで英語訳を表示（JMdict逆引き）
- [x] 日本語単語ロングプレスで文章モード（形態素解析）
- [x] 英語投稿と同じUX（タップ=単語、ロングプレス=文章）
- [x] 単体テスト作成
