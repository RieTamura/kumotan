#!/usr/bin/env node
/**
 * Script to generate src/constants/ngslsWords.ts from NGSL-S CSV file
 *
 * Source:
 *   - NGSL-Spoken_1.2_stats.csv  (Lemma, Rank, SFI, U)
 *
...
// Parse stats CSV → entries sorted by rank
...
// Generate TypeScript content using Map to avoid Biome noThenProperty lint errors
const lines = [
  '/**',
...
lines.push('  const band = NGSLS_MAP.get(word.toLowerCase());');
...
console.log(`  Band 3 (rank 481+):   ${entries.filter(([, r]) => r > 480).length} words`);
